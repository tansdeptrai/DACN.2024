<?php
if (isset($_POST['idSize'], $_POST['idCate']) && !empty($_POST['idSize']) && !empty($_POST['idCate'])){
    $id_user = '';
    ...
    $quanList = '';
    if ($id_size && $id_cate):
        $productVps = getRaw("SELECT id, product_id, color_id, size_id, price FROM variant_products WHERE cate_id = $id_cate AND size_id = $id_size");
         if ($productVps) { 
            $quanList = sizeof($productVps);
...
<div>
    ...
    <div class="mt-4 d-flex justify-content-between">
        ...
        <span style="font-weight: 600; font-size:15px; color:black; padding-top:10px; letter-spacing: 1px;">Số
            Lượng :<?php echo $quanList;?></span> <!-- số lượng tổng -->
    </div>
</div>
<!-- Product -->
    ...
    <div class="product-item2">
        ...
        <div class="inforProduct2">
            <div class="colorProduct d-flex mt-2 mb-1">
                <?php 
                    foreach ($productVps as $vp) :
                        if ($product['id'] == $vp['product_id'] && $id_size == $vp['size_id']):
                            $idColor = $vp['color_id'];
                ...
                <button class="btnColor" data-idvp="<?php echo $vp['id']; // id variant Product?>"
                    data-id="<?php echo $color['id'];?>" data-idpro="<?php echo $product['id'];?>" data-idsize="<?php echo $vp['size_id'];?>" style="background-color: <?php echo $color['color_code'];?>;">
                </button>
                <?php 
                        endif;
                    endforeach;
